...
type KnowledgeBase = {
  ...
  education: KnowledgeSection
  projects: KnowledgeSection
  research: KnowledgeSection
  skills: KnowledgeSection
  contact: KnowledgeSection
  website: KnowledgeSection
}

const junzheKnowledge: KnowledgeBase = {
  profile: {
    title: 'Profile',
    summary:
...
export function getRelevantJunzheContext(message: string): string {
  ...
  const selected = new Set<keyof KnowledgeBase>(['profile'])

  const addWhen = (section: keyof KnowledgeBase, keywords: string[]) => {
